...
ALLOWED_LETTERS = ('a'..'z').to_a.join.freeze
...
def mymix(s1, s2)
  ...
  max = []
  letters = (s1_counts.keys + s2_counts.keys).uniq
  letters.map { |letter|   max << select_maximum(letter, s1_counts, s2_counts) }
  sorted =   max.sort_by { |ss| [-ss.length, ss.prefix, ss.body] }
  sorted.map { |ss| "#{ss.prefix}#{ss.body}" }.join '/'
end

# prefix values can be 1,2,=
def select_maximum(letter, s1_counts, s2_counts)
  if (s1_counts[letter]).zero?
    OpenStruct.new prefix: '2:', body: letter * s2_counts[letter], length: s2_counts[letter]
  elsif (s2_counts[letter]).zero?
    OpenStruct.new prefix: '1:', body: letter * s1_counts[letter], length: s1_counts[letter]
  elsif s1_counts[letter] > s2_counts[letter]
    OpenStruct.new prefix: '1:', body: letter * s1_counts[letter], length: s1_counts[letter]
  elsif s1_counts[letter] < s2_counts[letter]
    OpenStruct.new prefix: '2:', body: letter * s2_counts[letter], length: s2_counts[letter]
  elsif s1_counts[letter] == s2_counts[letter]
    OpenStruct.new prefix: '=:', body: letter * s2_counts[letter], length: s2_counts[letter]
  end
end

def counts(string)
  h = Hash.new(0)
  s1_arry = string.chars.select { |ch| ALLOWED_LETTERS.include? ch }
  s1_arry.each { |e| h[e] += 1 }
  h.delete_if { |_k, v| v == 1 }
end

# This is the version most up voted
def mix(s1, s2)
